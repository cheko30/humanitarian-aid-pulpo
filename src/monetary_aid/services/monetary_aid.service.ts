...
export class MonetaryAidService {
  constructor(private responsibleAgenciesService: ResponsibleAgenciesService) {}
  ...
  async getMonetaryAidByConutryAndRangeYear(year: number, codeCountry: string) {
    ...
    const agencies = await this.responsibleAgenciesService.findAll();
    console.log(agencies.length);
    newArrayAgencies.reverse().forEach(async (element) => {
      element.responseSorted.forEach(async (e) => {
        const agecnyName = agencies.find(
          (agency) => agency.code === e.agencyref,
        );
        const agecnyNameUpper = this.agencyNameFormatter(
          agecnyName.description,
        );
        agencyCuantityUsdObj[agecnyNameUpper] = e.sum;
      });
